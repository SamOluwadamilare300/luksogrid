...
import { IconCalendar, IconChartLine, IconTrendingDown, IconTrendingUp } from "@tabler/icons-react"
...
export function CryptoDetailSheet({ crypto, open, onOpenChange }: CryptoDetailSheetProps) {
  ...
  const [cryptoData, setCryptoData] = React.useState<ChartDataItem[]>([])
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      setIsLoading(true)
      try {
        const historicalData = await CryptoDataService.getCryptoHistoricalData(crypto)
        const formattedData = CryptoDataService.formatChartData(historicalData)
  ...
  React.useEffect(() => {
    const filtered = CryptoDataService.filterDataByTimeRange(cryptoData, timeRange as "1m" | "3m" | "6m" | "1y" | "5y")
    setFilteredData(filtered)
  }, [timeRange, cryptoData])

  const chartChange = CryptoDataService.calculateChange(filteredData)
  const chartIsPositive = Number.parseFloat(chartChange.percent as string) >= 0
      ...
      <SheetContent className="w-full overflow-y-auto p-5">
        <SheetHeader className="px-1 pb-4">
          <div className="flex items-center justify-between">
            <div>
              <SheetTitle className="font-bold text-2xl">{crypto.symbol}</SheetTitle>
              <SheetDescription>{crypto.name}</SheetDescription>
            </div>
            <Badge
              variant="outline"
        ...
        <Tabs defaultValue="overview" className="mt-2">
          ...
          <TabsContent value="overview" className="mt-4 space-y-6 px-1">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-1">
                ...
                <p className="font-medium text-xl">${crypto.price.toFixed(2)}</p>
              </div>
              <div className="space-y-1">
                <p className="text-muted-foreground text-sm">Change (24h)</p>
                <p className={`font-medium ${isPositive ? "text-green-500" : "text-red-500"}`}>
                  ${crypto.change.toFixed(2)} ({formatPercent(crypto.changePercent)})
                </p>
              </div>
              <div className="space-y-1">
                <p className="text-muted-foreground text-sm">Market Cap</p>
                <p className="">{formatMarketCap(crypto.marketCap)}</p>
              </div>
              <div className="space-y-1">
          ...
          <TabsContent value="chart" className="mt-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant="outline"
                className={`flex items-center gap-1 ${chartIsPositive ? "text-green-500" : "text-red-500"}`}
              >
                {chartIsPositive ? <IconTrendingUp className="size-4" /> : <IconTrendingDown className="size-4" />}
                {chartIsPositive ? "+" : ""}
                {chartChange.percent}%
              </Badge>
            </div>
              ...
              {isLoading ? (
                ...
                <ChartContainer config={chartConfig} className="aspect-auto h-full w-full">
                  <AreaChart data={filteredData}>
                    <defs>
                      <linearGradient id="fillCryptoPrice" x1="0" y1="0" x2="0" y2="1">
                        <stop
                          offset="5%"
                          stopColor={chartIsPositive ? "rgba(34, 197, 94, 0.8)" : "rgba(239, 68, 68, 0.8)"}
                          stopOpacity={0.8}
                        />
                        <stop
                          offset="95%"
                          stopColor={chartIsPositive ? "rgba(34, 197, 94, 0.1)" : "rgba(239, 68, 68, 0.1)"}
                          stopOpacity={0.1}
                        />
                      </linearGradient>
                    </defs>
                    ...
                    <Area
                      dataKey="price"
                      type="monotone"
                      fill="url(#fillCryptoPrice)"
                      stroke={chartIsPositive ? "rgba(34, 197, 94, 1)" : "rgba(239, 68, 68, 1)"}
                      strokeWidth={2}
                    />
                  </AreaChart>
                </ChartContainer>
